...
function renderForm(req,res,locals){
  res.render('profile', extend({
    ...
    card_number: req.user.customData.card_number,
    card_security_code: req.user.customData.card_security_code,
    card_expiration_date: req.user.customData.card_expiration_date,
    disable_ads: req.user.customData.disable_ads === '1' ? 'checked' : false,
    is_visitor: req.user.customData.account_type == consts.ACCOUNT_VISITOR
  },locals||{}));
}
  ...
  router.all('/', stormpath.loginRequired, function(req, res) {
    profileForm.handle(req,{
      success: function(form){
        ...
        req.user.customData.card_number = form.data.card_number;
        req.user.customData.card_security_code = form.data.card_security_code;
        req.user.customData.card_expiration_date = form.data.card_expiration_date;
        req.user.customData.disable_ads = (form.data.disable_ads == 'on') ? '1' : '0';
